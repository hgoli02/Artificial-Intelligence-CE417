...
class BoardUtility:
    ...
    def score_position(game_board, piece):
        ...
        two_tile_score = 10
        three_tile_score = 100
        for i in range(ROWS):
            for j in range(COLS):
                try:
                    if game_board[i][j] == piece and game_board[i][j + 1] == piece:
                        score += two_tile_score
                    if game_board[i][j] == piece and game_board[i + 1][j] == piece:
                        score += two_tile_score
                    if game_board[i][j] == piece and game_board[i + 1][j + 1] == piece:
                        score += two_tile_score
                    if game_board[i][j] == piece and game_board[i + 1][j - 1] == piece:
                        score += two_tile_score
                    if game_board[i][j] == opponent_piece and game_board[i][j + 1] == opponent_piece:
                        score -= two_tile_score
                    if game_board[i][j] == opponent_piece and game_board[i + 1][j] == opponent_piece:
                        score -= two_tile_score
                    if game_board[i][j] == opponent_piece and game_board[i + 1][j + 1] == opponent_piece:
                        score -= two_tile_score
                    if game_board[i][j] == opponent_piece and game_board[i + 1][j - 1] == opponent_piece:
                        score -= two_tile_score
                except IndexError:
                ...
                try:
                    if game_board[i][j] == piece and game_board[i][j + 1] == piece and game_board[i][j + 2] == piece:
                        score += three_tile_score
                    if game_board[i][j] == piece and game_board[i + 1][j] == piece and game_board[i + 2][j] == piece:
                        score += three_tile_score
                    if game_board[i][j] == piece and game_board[i + 1][j + 1] == piece and game_board[i + 2][j + 2] == piece:
                        score += three_tile_score
                    if game_board[i][j] == piece and game_board[i + 1][j - 1] == piece and game_board[i + 2][j - 2] == piece:
                        score += three_tile_score
                    if game_board[i][j] == opponent_piece and game_board[i][j + 1] == opponent_piece and game_board[i][j + 2] == opponent_piece:
                        score -= three_tile_score
                    if game_board[i][j] == opponent_piece and game_board[i + 1][j] == opponent_piece and game_board[i + 2][j] == opponent_piece:
                        score -= three_tile_score
                    if game_board[i][j] == opponent_piece and game_board[i + 1][j + 1] == opponent_piece and game_board[i + 2][j + 2] == opponent_piece:
                        score -= three_tile_score
                    if game_board[i][j] == opponent_piece and game_board[i + 1][j - 1] == opponent_piece and game_board[i + 2][j - 2] == opponent_piece:
                        score -= three_tile_score
                except IndexError:
